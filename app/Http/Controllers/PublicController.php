...
class PublicController extends Controller
{
    ...
    public function spanishAllStandings() { $tournament = Tournament::query()->where('status', 'ACTIVE')->firstOrFail(); return response()->json(['data' => $tournament->categories()->where('is_active', true)->orderBy('sort_order')->get(['categories.id', 'categories.name'])->mapWithKeys(function (Category $category) use ($tournament) { $rows = $this->standingsData($tournament, $category->id)['standings']; return [$category->name => array_map(fn ($row, $index) => ['posicion' => $index + 1, 'club' => $row['club_name'], 'puntos' => $row['points'], 'pj' => $row['played'], 'pg' => $row['won'], 'pe' => $row['drawn'], 'pp' => $row['lost'], 'gf' => $row['goals_for'], 'gc' => $row['goals_against'], 'dg' => $row['goal_difference']], $rows, array_keys($rows))]; })->all()]); }
